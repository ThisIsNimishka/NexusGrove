...
import { Palette, Sparkles, SlidersHorizontal, Terminal, Network, Unplug, RefreshCw, Settings2, Zap, Scale, Target, CodeXml, PanelRightClose, PanelRightOpen, Settings } from 'lucide-react'
...
export function SettingsSidebar() {
  ...
  return (
    <aside
      className={cn(
        "glass border-l border-white/10 flex flex-col z-30 transition-butter duration-butter relative group/sidebar",
        settingsSidebarCollapsed ? "w-16" : "w-80"
      )}
    >
      ...
      <button
        ...
        className={cn(
          "absolute -left-3 top-20 z-40 w-6 h-12 flex items-center justify-center glass border border-white/10 rounded-full text-muted-foreground hover:text-primary transition-all duration-300 opacity-0 group-hover/sidebar:opacity-100",
          settingsSidebarCollapsed && "opacity-100"
        )}
      >
        {settingsSidebarCollapsed ? <PanelRightOpen className="w-4 h-4" /> : <PanelRightClose className="w-4 h-4" />}
      </button>
              ...
              <div className="flex items-center gap-2.5 flex-1">
                {isConnected ? (
                  <>
                    ...
                    <span className="font-semibold text-foreground/90">Connected</span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="font-semibold text-foreground/90">Offline</span>
                  </>
                )}
              </div>
        ...
        <div className="flex-1 flex flex-col items-center py-6 gap-6">
          ...
          <div className="flex flex-col items-center gap-3 mt-auto mb-6">
            <div className={cn(
              ...
              isConnected ? "bg-accent-cyan shadow-[0_0_8px_hsl(var(--accent-cyan))]" : "bg-destructive shadow-[0_0_8px_hsl(var(--destructive))]"
            )} />
            {isConnected && <Network className="w-4 h-4 text-accent-cyan/40" />}
          </div>
        </div>
      )}
